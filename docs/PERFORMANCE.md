...
| 경로 | 최적화 |
|---|---|
| Conductance | 그래프 수를 CPU 메타데이터에서 읽어 층별 GPU scalar 조회 제거; split 인덱스 1회 준비; loss/PPI 지표 device 누적 |
...
v2는 전체 signed 기저를 그대로 사용한다. `f(u)`/`f(-u)` 평균, 열별 전체 엣지 context,
...
이 값으로 제한한다는 뜻은 아니다. 원래 v2 실행 경로와 비교하려면 다음처럼 실행한다.
...
기본 데이터는 Conductance의 Cora, v2의 ZINC-12K다. `--dataset`으로 해당 트랙의 다른
...
- Conductance reference는 층별 graph-count scalar 조회가 있던 경로다. 모든 이전 코드와의
  end-to-end 비교는 아니다. v2는 reference/batched 기저 인코더를 비교한다.

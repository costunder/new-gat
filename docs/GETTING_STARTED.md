...
현재 V5·Cycle V2 SE/PE의 통합 실행은
[실제 batch/worker 측정 후 학습하는 최신 실행 안내](../gpt_handoff/RICH_SCALING_EXPERIMENTS.md)를
우선한다. 완료한 V1–V4·Cycle V1·Tree를 다시 실행하는 명령이 아니다. 서버에서 같은 저장소의
학습이 진행 중이라면 소스 무결성 검사를 깨뜨릴 수 있으므로 그 폴더에서 `git pull`하지 않는다.
기존 V5 진행분을 새 C 구조로 옮길 때는 위 안내의 `run_v5_transition.py`를 사용한다.
원본 checkpoint 쓰기가 멈춘 것을 확인한 뒤 별도 출력에 공통 가중치·AdamW 상태·누적 epoch를
유지하고 C만 교체한다. 완료 fixed 결과는 재학습하지 않는다. 기존 결과를 삭제하거나
서로 다른 구조의 checkpoint를 일반 resume로 강제 로드하지 않는다.

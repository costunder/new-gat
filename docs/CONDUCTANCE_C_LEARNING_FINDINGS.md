...
## 7. 2026-09-01 당시 다음 독립 가설: 직접 C v2와 상대 C 생성기 v3
...
PPI의 unseen 독립 그래프에는 적용하지 않는다. 이 기록을 작성한 2026-09-01 당시 계획은
arxiv × direct/fixed × seed 0의 2개 CUDA 학습이었고 GPU 결과도 아직 없었다. 현재 기본 계약은
Cora/CiteSeer/PubMed/ogbn-arxiv × direct/fixed × seed 0의 8개 full-graph 학습이며 PPI는 N/A다.
2026-09-02의 과거 arxiv-only V2 runner `passed` 보고는 있지만 성능 수치와 전체 artifact는
수령하지 않았고, 현재 8개 전체 실행의 결과로 재사용하지 않는다.
...
당시 계획은 같은 arxiv와 seed 0에서 `relative_c`/`fixed_c`를 새로 학습하는 것이었다.
현재 기본 계약은 Cora/CiteSeer/PubMed/PPI/ogbn-arxiv × 두 조건 × seed 0의 10개 학습이다.
PPI는 공식 20/2/2 독립 graph split, whole-graph minibatch 2, BCEWithLogits와 global node-label
micro-F1을 사용하며 fixed 조건도 alpha는 학습한다.
...
optimizer가 함께 달라 전체 점수 차이를 C 한 요인의 효과로 해석하지 않는다. 2026-09-02의
과거 arxiv-only V3 runner `passed` 보고는 있지만 성능 수치와 전체 artifact는 수령하지 않았고,
현재 10개 전체 실행 결과도 아직 없다. 단일 seed validation 결과를 유의성·test·SOTA 주장으로
확대하지 않는다.

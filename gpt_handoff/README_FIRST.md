...
GPT에는 파일을 따로 고르지 말고 이 폴더의 **8개 파일을 전부** 전달한다.
...
8. `CODE_SUMMARY.md`: 현재 Python·test·config·script 200개 전체의 원문 스냅샷

Conductance v2/v3/v4와 Cycle PE v2는 각각의 원문 문서를 직접 제공한다. 이 네 문서만 보는
것도 아니며 Conductance v1, Cycle PE v1, Tree Augmentation, 데이터·평가 계약과 과거 GPU 결과는
`HANDOFF.md`와 `EXPERIMENT_STATUS.md`에서 함께 검토한다.

## GPT에 그대로 줄 요청문

> 첨부한 `gpt_handoff` 폴더의 8개 파일을 모두 읽고 NEW GAT 전체 프로젝트를 교차검증해 줘.
> V4만 검토하지 말고 Conductance v1/v2/v3/v4, Cycle PE v1/v2, Tree Augmentation과 공통
> 데이터·평가·실행 계약을 모두 범위에 포함해라. 문서의 주장과 `CODE_SUMMARY.md`의 실제 구현이
